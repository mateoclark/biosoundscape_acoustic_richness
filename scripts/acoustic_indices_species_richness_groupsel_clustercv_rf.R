# Purpose: Random forest modeling of species richness from acoustic indices statistics
...
predictor_files <- c(
  ...
  # Acoustic indices descriptive statistics
  
  "acoustic_indices_minutes_dawn-dusk_AGI90_descriptive_statistics.csv",
  "acoustic_indices_minutes_dawn-dusk_AGI75_descriptive_statistics.csv",
  "acoustic_indices_minutes_dawn-dusk_AGI50_descriptive_statistics.csv",
  "acoustic_indices_minutes_dawn-dusk_descriptive_statistics.csv",

  # Acoustic indices harmonic statistics

  "acoustic_indices_minutes_AGI90_2phase-harmonic_statistics.csv",
  "acoustic_indices_minutes_AGI75_2phase-harmonic_statistics.csv",
  "acoustic_indices_minutes_AGI50_2phase-harmonic_statistics.csv",
  "acoustic_indices_minutes_2phase-harmonic_statistics.csv"
)
...
for (predictor_file in predictor_files){
  ...
  dry_data <- species_filtered %>%
    ...
    select(PCrichness, AIrichness, CLUSTER_ID, LogRecNum, matches(pattern))
  ...
  predictor_vars <- df %>% select(-SiteID) %>% colnames()
  ...
  save(dry_models, file = file.path(inDir,"models/acoustic_indices",outfile))
  
  outfile <- sub("\\.csv$", "_wet_season.RData", predictor_file)
  save(wet_models, file = file.path(inDir,"models/acoustic_indices",outfile))
  
  outfile <- sub("\\.csv$", "_dry-wet_season.RData", predictor_file)
  save(drywet_models, file = file.path(inDir,"models/acoustic_indices",outfile))
