fit_rfe_cluster_rf <- function(cor_data, predictor_vars, metric,
  ...
  filtered_data <- df_rf %>% dplyr::select(all_of(predictor_vars), response, cluster_id)
  ...
  if (rfe_on == 1) {
    ctrl <- rfeControl(functions = caretFuncs, method = "cv", number = 3, allowParallel = FALSE)
