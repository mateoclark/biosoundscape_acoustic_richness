# Purpose: Random forest modeling of species richness from BirdNET embedding PCA statistics
# Matthew Clark, April 1, 2026
...
predictor_files <- c(
  "birdnet_pca_minutes_nonzero_mean_dawn-dusk_descriptive_statistics.csv",
  "birdnet_pca_minutes_nonzero_mean_2phase-harmonic_statistics.csv"
)
...
for (predictor_file in predictor_files){
  ...
  dry_data <- species_filtered %>%
    ...
    select(PCrichness, AIrichness, CLUSTER_ID, LogRecNum, matches("^PC"))
  ...
  predictor_vars <- dry_data %>% select(matches("PC")) %>% select(-PCrichness) %>% colnames() 
  ...
  save(dry_models, file = file.path(inDir,"models/rf_pca",outfile))
  
  outfile <- sub("\\.csv$", "_wet_season.RData", predictor_file)
  save(wet_models, file = file.path(inDir,"models/rf_pca",outfile))
  
  outfile <- sub("\\.csv$", "_dry-wet_season.RData", predictor_file)
  save(drywet_models, file = file.path(inDir,"models/rf_pca",outfile))
