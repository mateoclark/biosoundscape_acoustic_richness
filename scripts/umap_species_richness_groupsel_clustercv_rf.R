...
umapDir <- "G:/Shared drives/BioSoundSCape/Paper development/RQ1.1 Animal-acoustic diversity relationships/umap_tpd_modeling/BirdNET_pca_umap_stats"
...
diversity_vars <- c("AIrichness")

# BirdNET embedding PCA UMAP descriptive statistics files

predictor_files <- NULL
for (i in 1:nrow(param_grid)) {
  params <- param_grid[i, ]
  nn <- params$n_neighbors
  md <- params$min_dist
  predictor_files <- c(predictor_files,
                       paste0("birdnet_embeddings_minute_nonzero_mean_dry-wet_pca_umap_nn",nn,"_mdist",md,"_2phase-harmonic_statistics.csv"),
                       paste0("birdnet_embeddings_minute_nonzero_mean_dawn-dusk_dry-wet_pca_umap_nn",nn,"_mdist",md,"_descriptive_statistics.csv"))
}
...
for (predictor_file in predictor_files){
  ...
  df <- read.csv(file.path(umapDir,predictor_file))
  ...
  dry_data <- species_filtered %>%
    ...
    select(PCrichness, AIrichness, CLUSTER_ID, LogRecNum, matches("^UMAP"))
  ...
  predictor_vars <- dry_data %>% select(matches("UMAP")) %>% colnames() 
  ...
  save(dry_models, file = file.path(inDir,"models/rf_umap",outfile))
  
  outfile <- sub("\\.csv$", "_wet_season.RData", predictor_file)
  save(wet_models, file = file.path(inDir,"models/rf_umap",outfile))
  
  outfile <- sub("\\.csv$", "_dry-wet_season.RData", predictor_file)
  save(drywet_models, file = file.path(inDir,"models/rf_umap",outfile))
